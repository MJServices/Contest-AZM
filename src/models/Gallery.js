...
const Gallery = sequelize.define(
  "galleries",
  {
    ...
    image_url: {
      ...
      validate: {
        validUrl(value) {
          // Check if it's a development URL (localhost or IP)
          const devUrlRegex =
          ...
          if (!devUrlRegex.test(value) && !prodUrlRegex.test(value)) {
            throw new Error(
    ...
    thumbnail_url: {
      ...
      validate: {
        validUrl(value) {
          if (value) {
            // Only validate if a value is provided since this field is allowNull: true
            // Check if it's a development URL (localhost or IP)
            const devUrlRegex =
              /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?(\/.*)?$/i;
            // Check if it's a production URL
