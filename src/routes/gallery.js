...
// POST /api/v1/gallery - Upload new gallery item (any authenticated user can upload)
router.post(
  "/",
  authenticateToken,
  upload.single("image"),
  async (req, res) => {
    try {
      ...
      if (req.user.role === "user") {
        try {
          // Update user role to designer-pending
          await User.update(
          ...
          console.log(
            `User ${req.user.user_id} role upgraded to designer-pending after uploading design`
          );
        } catch (roleUpdateError) {
          console.error("Error updating user role:", roleUpdateError);
          // Don't fail the upload if role update fails
        }
      }
      ...
      res.status(201).json({
        ...
        message:
          req.user.role === "admin"
            ...
            : "Gallery item uploaded and pending approval",
        data: createdItem,
      });
    } catch (error) {
...
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (
      req.user.role !== "admin" &&
      galleryItem.uploaded_by !== req.user.user_id
    ) {
    ...
    try {
      ...
      if (color_palette !== undefined)
        parsedColorPalette = JSON.parse(color_palette);
    } catch (parseError) {
      return res.status(400).json({
        success: false,
        message: "Invalid JSON format for tags or color_palette",
    ...
    if (color_palette !== undefined)
      updateData.color_palette = parsedColorPalette;
